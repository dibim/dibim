 ...
 *
 */
export async function connectMssql(connName: string, p: DbConnectionParam) {
  return await invoker.connectSql(connName, `mssql://${p.user}:${p.password}@${p.host}:${p.port}/${p.dbName}`);
}

// 获取所有表格名
export async function getAllTableNameMssql(connName: string) {
  const sql = `
    SELECT 
...
export async function getAllTableSizeMssql(connName: string) {
  const sql = `
    SELECT
...
// 获取表结构
export async function getTableStructureMssql(connName: string, tbName: string) {
  // 基础字段信息
  const columnSql = `
    SELECT
      c.name AS [name],
...
// 获取表格的 DDL
export async function getTableDdlMssql(connName: string, tbName: string) {
  const sql = `
    SELECT 
 ...
 * @returns
 */
export async function getPageCountMssql(connName: string, tableName: string, pageSize: number, condition: string) {
...
// 获取表格数据
export async function getTableDataMssql(connName: string, p: GetTableDataParam) {
  const { itemsTotal, pageTotal } = await getPageCountMssql(connName, p.tableName, p.pageSize, "");

  let offset = p.pageSize * (p.currentPage - 1);
  if (offset < 0) offset = 0;
  const sqlHasPkey = `SELECT * FROM [${p.tableName}] ${p.where} ORDER BY (SELECT NULL) OFFSET ${offset} ROWS FETCH NEXT ${p.pageSize} ROWS ONLY;`;
...
// 生成重命名表格的语句
export function genRenameTableCmdMssql(oldName: string, newName: string) {
  return `EXEC sp_rename '[${oldName}]', '${newName}';`;
}

// 生成截断表格的语句
export function genTruncateTableCmdMssql(tbName: string) {
  return `TRUNCATE TABLE [${tbName}];`;
}

// 生成删除表格的语句
export function genDeleteTableCmdMssql(tbName: string) {
  return `DROP TABLE [${tbName}];`;
}

// 生成重命名字段的语句
export function genRenameFieldCmdMssql(tbName: string, oldName: string, newName: string) {
  return `EXEC sp_rename '[${tbName}].[${oldName}]', '${newName}', 'COLUMN';`;
}

// 生成删除字段的语句
export function genDeleteFieldCmdMssql(tbName: string, fieldName: string) {
  return `ALTER TABLE [${tbName}] DROP COLUMN [${fieldName}];`;
}

// 生成变更一行的字段
export function genUpdateFieldCmdMssql(tbName: string, uniqueField: FieldWithValue, fieldArr: FieldWithValue[]) {
...
// 生成删除多行的字段
export function genDeleteRowsCmdMssql(tbName: string, fieldName: string, fieldValues: any[]) {
  const values = fieldValues.map((item) => formatToSqlValueMssql(item)).join(",");
  return `DELETE FROM [${tbName}] WHERE [${fieldName}] IN (${values});`;
}

// 生成插入多行数据
export function genInsertRowsCmdMssql(tbName: string, fieldNames: string[], fieldValues: any[][]) {
